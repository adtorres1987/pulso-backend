...
export class GetGroupExpensesUseCase {
  ...
  async execute(groupId: string, userId: string, page: number, limit: number, month?: string): Promise<PaginatedGroupExpenses> {
    const group = await this.repo.findByIdAndUser(groupId, userId);
    if (!group) throw new AppError('Group not found', 404);
    return this.repo.findExpensesByGroup(groupId, page, limit, month);
  }
}
